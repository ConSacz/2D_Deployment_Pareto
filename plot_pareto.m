...
clear rawData;
%% load data
load ('./pareto old/ran_pop.mat')
for j=minIt:100:maxIt
    name =['./pareto old/' num2str(j) '.mat'];
    load (name)
    for i =1:nPop
        G=Graph(pop(i).Position,Rc);
        Data((j-minIt)/100*nPop+i).Cost1=Cov_Func_v2(pop(i).Position,Rs,Obstacle_Area,Covered_Area) ;
        Data((j-minIt)/100*nPop+i).Cost2=Energy_consumption(G);
        Data((j-minIt)/100*nPop+i).Cost2=Life_Time(G);
    end
...
for i=2:allpop          %chay giua cac diem trong Data
    for j=1:numel(a1)
        ...
        if checkDomination(Data(a1(j)),Data(i)) == 1
            count=0;
            break;
        % if pop(i) dominated a1
        elseif checkDomination(Data(a1(j)),Data(i)) == -1
            count=0;
            a1(j)=i;
        % nondominated
        else
...
for i=1:allpop
    if PRT2(i)==0
        ...
        text (Data(i).Cost1 , Data(i).Cost2, num2str(i),"FontSize",10)
        hold on;
    end

    if PRT2(i)==1
        a2=[a2;i];
        plot (Data(i).Cost1 , Data(i).Cost2,'ro','Color',"g");
        %text (Data(i).Cost1 , Data(i).Cost2, num2str(i),"FontSize",10)
        hold on;
    end
    if PRT2(i)==2
...
xlim([0 1])
ylim([0 1])
plot([0.2, 0.2], [0, 1], 'k--', 'LineWidth', 0.05);
hold on;
for i=1:numel(a1)
     %if a1(i)== 4967
     ...
     %else
        plot3 (Data(a1(i)).Cost1,Data(a1(i)).Cost2,Data(a1(i)).Cost3, 'ro','Color','g');
     %end
